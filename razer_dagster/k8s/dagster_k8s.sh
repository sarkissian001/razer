...
# Set default values
default_chart_name="dagster"
default_chart_version="1.1.20"
...
install_chart() {
  if helm ls --all-namespaces | grep -q "^$chart_name\s"; then
    ...
    if [ -n "$namespace" ]; then
      ...
      # TODO: add release name instead of hard code it to dagster/$chart_name
      helm install $chart_name dagster/$chart_name --version $chart_version -f ./helm/values.yaml --namespace $namespace
    else
      echo "Installing $chart_name"
      # TODO: add release name instead of hard code it to dagster/$chart_name
      helm install $chart_name dagster/$chart_name --version $chart_version -f ./helm/values.yaml
    fi
  fi
}
...
if [ "$1" = "-i" ]; then
  ...
  if ! helm repo list | awk '{print $1}' | grep -q "^dagster"; then
    # Add repository
    echo "Adding Dagster charts to helm repo (charts: https://dagster-io.github.io/helm)"
    helm repo add dagster https://dagster-io.github.io/helm

    helm repo update
  else
    echo "Dagster repository found."
  fi
